...
public:
	...
	btRigidBody* createRigidBodyFromMesh(float mass, float x, float y, float z) {
		btTransform t;
		t.setIdentity();
		...
		for(int i = 0; i < meshes.size(); i++){
			...
			for(int j = 0; j < vertices.size(); j++){
				shape->addPoint(btVector3(vertices[j].Position.x, vertices[j].Position.y, vertices[j].Position.z));
			}
		}
