...
public:
	...
	Mesh(const aiMesh* assimpMesh, const aiNode* meshNode, aiNode* rootNode, const aiMatrix4x4& transform);
	Mesh(const Mesh& rightHand);
	Mesh& operator=(const Mesh& rightHand);
	~Mesh();
	...
	size_t getBoneIndex(const std::string& name) const { return m_boneNameToIndex.at(name); }
	Bone* getBoneByName(const std::string& name) const { return m_bonesByName.at(name); }
