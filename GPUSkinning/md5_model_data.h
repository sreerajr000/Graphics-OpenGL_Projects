...
public:
	...
	void CreateTextures ()
	{
		...
		texture_arr_ = new TextureAnim*[mesh_num_];

		for (int i = 0; i < mesh_num_; ++i)
		{
			std::cout << omesh_arr_[i].texture_name_.c_str () << std::endl;
			texture_arr_[i] = new TextureAnim (omesh_arr_[i].texture_name_.c_str ());
		}
	}
